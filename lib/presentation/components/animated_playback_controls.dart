...
class _AnimatedPlaybackControlsState extends State<AnimatedPlaybackControls> {
    ...
    return SizedBox(
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          final totalWeight = _getWeight(PlaybackButton.prev) + 
          ...
          return Row(
            ...
            children: [
               _buildButton(
                 width: availableWidth * (_getWeight(PlaybackButton.prev) / totalWeight),
               ...
               _buildButton(
                 width: availableWidth * (_getWeight(PlaybackButton.playPause) / totalWeight),
                 icon: widget.isPlaying ? Icons.pause_rounded : Icons.play_arrow_rounded,
                 color: AppColors.pillPaleGreen,
                 isPlayPause: true,
                 onTap: () => _handleTap(PlaybackButton.playPause, widget.onPlayPause),
               ),
               _buildButton(
                 width: availableWidth * (_getWeight(PlaybackButton.next) / totalWeight),
  ...
  Widget _buildButton({
    ...
    final borderRadius = isPlayPause 
        ? (widget.isPlaying ? 26.0 : 60.0) // Squircle when playing, Circle when paused
        ...
        child: Center(
          child: AnimatedSwitcher(
            ...
            child: Icon(
              ...
              key: ValueKey(icon), 
              size: 36, 
              color: AppColors.playerBackground // Deep brown/black icon to match theme
            ),
          ),
        ),
      ),
